...
type Events = {
  switchChat: { sessionId: string; title: string }
  createChat: void
  deleteChat: string  // 删除会话事件，参数为会话ID
  renameChat: { sessionId: string; newTitle: string }  // 重命名会话事件，参数包含会话ID和新标题
}

const emitter = mitt<Events>()

// 切换聊天会话
export const switchToChat = (chatInfo: { sessionId: string; title: string }) => {
...
export const renameChat = (sessionId: string, newTitle: string) => {
  emitter.emit('renameChat', { sessionId, newTitle })
}

// 监听切换聊天事件
export const onSwitchChat = (callback: (chatInfo: { sessionId: string; title: string }) => void) => {
...
// 监听重命名聊天事件
export const onRenameChat = (callback: (info: { sessionId: string; newTitle: string }) => void) => {
